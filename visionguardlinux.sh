...
<body>
<div class="logo">
    <img src="/tmp/1.png" alt="Logo" width="200">
</div>
...
secure_passwords=$(awk -F: '($2 != "" && $2 != "*") {print $1}' /etc/shadow)
if [[ -n $secure_passwords ]]; then
    IFS=$'\n'
    for user in $secure_passwords; do
        password="<span style=\"color:green;\">Sécurisé</span>"
        echo "<tr><td>$user</td><td>$password</td></tr>" >> "$results_html"
    done
else
    echo "<tr><td>Aucun mot de passe sécurisé trouvé.</td><td></td></tr>" >> "$results_html"
fi
